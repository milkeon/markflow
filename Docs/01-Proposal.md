...
- 프로젝트 목표 — 4주 / 3인 팀으로 핵심 기능(MVP)을 완성하고, 실시간 협업을 데모 하이라이트로 시연한다.
...
| 담당 | 역할 | 주요 책임 |
| --- | --- | --- |
| **BE** | 백엔드 전체 | Prisma·REST·서비스·권한 **+ Socket.io 서버·룸·동기화·락·채팅** (기존 소켓·도메인 통합) |
| **F1** | 캔버스 / 실시간 | React Flow 캔버스·노드 연결·접기/펼치기, `useCollaboration`(소켓 클라)·멀티커서·소프트락 UI, Zustand 캔버스 스토어 |
| **F2** | 셸 / 콘텐츠 / 패널 | 인증·프로젝트 리스트, MD 상세 에디터, 채팅·히스토리 패널, 휴지통, API 클라이언트·통합 |

> 데모 안전장치 — BE가 Socket.io에서 막히면 동일한 CollabAPI 뒤에 Liveblocks 차선책으로 전환한다(UI 코드 불변). 3주차 소켓 부하는 F1이 클라이언트·오버레이로 분담한다.
